...
$out=Join-Path $env:TEMP ('velaguard-agent-'+[guid]::NewGuid().ToString('N'))
...
$src=@('tests\host\test_agent.c','app\velaguard\src\velaguard_tools.c','app\velaguard\src\velaguard_agent.c','app\velaguard\src\velaguard_core.c','app\velaguard\src\velaguard_store.c','app\velaguard\src\velaguard_runtime.c','app\velaguard\src\velaguard_commands.c') | ForEach-Object { Join-Path $repo $_ }
...
$src+=$toolObject
$exe=Join-Path $out 'agent_tests.exe'
